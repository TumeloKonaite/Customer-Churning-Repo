...
- `training_artifacts.py` stages the complete local artifact bundle and replaces a previous bundle only after the new one is ready.
- `dataset_identity.py` builds the lineage identities recorded with that bundle.
...
```text
DataIngestion -> ModelTrainer.fit -> TrainingArtifactWriter -> ExperimentTracker
                                                        -> exact registry version
                                                        -> deployment package -> Modal
```

Nothing in this folder fits or refits a model. DagsHub MLflow remains the only
remote source of truth, and the numeric registered-model version remains
authoritative. Pure fitting and evaluation live in `src/training/`; the training
pipeline coordinates ingestion, fitting, artifact publication, and tracking.
