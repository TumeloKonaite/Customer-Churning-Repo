...
export function BatchResults({ response }: { response: BatchResponse }) {
  ...
  return <section className="space-y-6" aria-live="polite">
    ...
    {response.results.length > 0 && <div className="card overflow-hidden">
      <div className="flex flex-col gap-4 border-b p-5 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="font-bold text-navy-900">Prediction results</h2><p className="mt-1 text-sm text-slate-500">Showing {results.length} of {response.results.length} scored rows</p></div><div className="flex flex-col gap-2 sm:flex-row"><label className="relative"><span className="sr-only">Filter results</span><Search className="pointer-events-none absolute left-3 top-3 h-4 w-4 text-slate-400" /><input className="field py-2 pl-9 text-sm" value={filter} onChange={(event) => setFilter(event.target.value)} placeholder="Filter by ID or risk" /></label><button className="button-secondary min-h-10 py-2" onClick={() => downloadText("churn-prediction-results.csv", resultsToCsv(response))}><Download className="h-4 w-4" /> Download CSV</button></div></div>
